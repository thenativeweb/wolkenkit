...
suite('setup store consumer-progress mysql', function () {
    this.timeout(30000);
    test(`sets up a mysql database for a consumer progress store.`, async () => {
        ...
        const { stdout } = shelljs_1.default.exec(setupMySqlConsumerProgressStoreCommand, { silent: false });
        assertthat_1.assert.that(stdout).is.containing('Successfully set up the MySQL consumer progress store.');
