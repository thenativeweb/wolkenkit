...
const domainEventSchemaParser = new validate_value_1.Parser(getDomainEventSchema_1.getDomainEventSchema());
const processDomainEvent = async function ({ application, domainEventDispatcher, consumerProgressStore, lockStore, repository, issueCommand, performReplay }) {
    ...
    try {
        domainEventSchemaParser.parse(domainEvent, { valueName: 'domainEvent' }).unwrapOrThrow((err) => new errors.DomainEventMalformed(err.message));
