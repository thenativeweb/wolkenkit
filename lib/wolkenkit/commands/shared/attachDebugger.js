...
const attachDebugger = async function (options, progress) {
  ...
  for (let i = 0; i < containers.length; i++) {
    ...
    const { devtoolsFrontendUrl, id } = debugConfiguration[0];
    const hash = devtoolsFrontendUrl.match(/@\w+/)[0];

    const debugUrl = `chrome-devtools://devtools/remote/serve_file/${hash}/inspector.html?experiments=true&v8only=true&ws=${host}:${debugPort}/${id}`;
