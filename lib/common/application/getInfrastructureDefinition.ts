...
const getInfrastructureDefinition = async function ({ infrastructureDirectory }: {
  ...
  if (!await exists({ path: infrastructureDirectory })) {
    throw new errors.DirectoryNotFound(`Directory '<app>/build/infrastructure' not found.`);
  }
  ...
  try {
    ...
    if (ex instanceof SyntaxError) {
      throw new errors.ApplicationMalformed(`Syntax error in '<app>/build/infrastructure'.`, { cause: ex });
    }

    // But throw an error if the entry is a directory without importable content.
    throw new errors.FileNotFound(`No infrastructure definition in '<app>/build/infrastructure' found.`);
  }
