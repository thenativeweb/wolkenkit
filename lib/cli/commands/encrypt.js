  ...
  async run (options) {
    ...
    if (help) {
      return buntstift.info(getUsage([
        ...
        { header: 'Synopsis', content: 'wolkenkit encrypt' },
    ...
    try {
      encrypted = await wolkenkit.encrypt({ env, directory, privateKey, value }, showProgress(verbose, stopWaiting));
    } catch (ex) {
      stopWaiting();
    ...
    stopWaiting();
    buntstift.success(`Encrypted value: ${encrypted}`);
  }
};
