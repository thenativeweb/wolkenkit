...
class PostgresNoLockDomainEventStore implements DomainEventStore {
  ...
  public static async create ({
    ...
    tableNames: TableNames;
  }): Promise<PostgresNoLockDomainEventStore> {
    const pool = new Pool({
      host: hostName,
      port,
      user: userName,
    ...
    disconnectWatcher.on('end', PostgresNoLockDomainEventStore.onUnexpectedClose);
    ...
    const domainEventStore = new PostgresNoLockDomainEventStore({
      pool,
      tableNames,
      disconnectWatcher
    });

    const connection = await PostgresNoLockDomainEventStore.getDatabase(pool);
  ...
  public async destroy (): Promise<void> {
    this.disconnectWatcher.removeListener('end', PostgresNoLockDomainEventStore.onUnexpectedClose);
  ...
  public async getLastDomainEvent <TDomainEventData extends DomainEventData> ({ aggregateIdentifier }: {
    ...
    const connection = await PostgresNoLockDomainEventStore.getDatabase(this.pool);
